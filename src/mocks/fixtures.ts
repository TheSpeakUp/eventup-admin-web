...
export function buildFixtureServices(): ServiceDetail[] {
  ...
  out.push({
    ...
    publication_discount_enabled: false,
    publication_discount_percent: null,
    publication_discount_promo_code: null,
    external_url: null,
    address: null,
    attributes: null,
  ...
  for (let i = 0; i < 25; i++) {
    ...
    out.push({
      ...
      provider_name: `Provider ${providerId} Studio`,
      status,
      category_id: ((i % 6) + 1),
      category_name: pick(CATEGORIES, i),
      recipient_type: i % 3,
      ...
      pricing_interval_minutes: null,
      max_units_per_order: null,
      publication_discount_enabled: i % 4 === 0,
      publication_discount_percent: i % 4 === 0 ? 10 + (i % 5) : null,
      publication_discount_promo_code: i % 4 === 0 ? `PROMO${i}` : null,
      external_url: i % 3 === 0 ? `https://example.com/service-${i + 1}` : null,
      address: i % 2 === 0 ? `${200 + i} Event Plaza` : null,
      attributes: null,
